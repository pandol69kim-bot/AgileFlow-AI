...
async def run_designer(state: dict) -> dict:
    ...
    design_system = await invoke_with_retry(llm, [HumanMessage(content=f"""당신은 시니어 UI/UX 디자이너입니다.
...
{state['product_requirements'][:3000]}""")])
    ...
    wireframes = await invoke_with_retry(llm, [HumanMessage(content=f"""다음 기획서의 주요 화면 3개를 ASCII 와이어프레임으로 작성하세요.
[기획서]
{state['product_requirements'][:2000]}""")])

    components = await invoke_with_retry(llm, [HumanMessage(content=f"""Atomic Design 기반 컴포넌트 명세를 작성하세요.
