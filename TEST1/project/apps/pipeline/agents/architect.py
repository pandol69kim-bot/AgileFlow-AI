...
async def run_architect(state: dict) -> dict:
    ...
    tech_stack = await invoke_with_retry(llm, [HumanMessage(content=f"""당신은 15년차 시니어 소프트웨어 아키텍트입니다.
...
{state['product_requirements'][:3000]}""")])
    ...
    architecture = await invoke_with_retry(llm, [HumanMessage(content=f"""시스템 아키텍처 문서를 작성하세요.
전체 구성도(ASCII), 서비스간 통신, 인증 흐름, API 설계 원칙 포함.
[기획서]
{state['product_requirements'][:2000]}""")])

    db_schema = await invoke_with_retry(llm, [HumanMessage(content=f"""데이터베이스 스키마를 작성하세요.
ERD 요약, 테이블 정의(컬럼명/타입/제약조건/설명), 인덱스 전략, 마이그레이션 계획 포함.
[기획서]
{state['product_requirements'][:2000]}""")])

    proj_structure = await invoke_with_retry(llm, [HumanMessage(content=f"""Turborepo 모노레포 디렉토리 구조를 작성하세요.
