...
**구현 내용**
실행 중인 파이프라인을 즉시 취소하는 기능 추가. 취소 시 현재까지 완료된 단계의 산출물은 보존됨.
...
**프롬프트**
```
http://localhost:5173/api/v1/projects/22f1bf4e-559f-426f-b06a-ce6064f5ca85/download   zip 다운로드 버튼 정상작동 여부 체크 후 수정해줘
```
...
**확인 방법**
```bash
# 1. API 서버 실행 후 프로젝트 완료 상태에서 ZIP 다운로드 버튼 클릭
# 2. 브라우저 Network 탭에서 /download 요청 → 200 응답 + zip 파일 다운로드 확인
```
...
**확인 방법**
```
파이프라인 완료 후 "↓ ZIP 다운로드" 클릭 → {project-slug}.zip 다운로드 → 폴더 구조 확인
```
...
**오류 내용**
```
http://localhost:5173/pipeline/undefined/projects/22f1bf4e-559f-426f-b06a-ce6064f5ca85/download
...
**확인 방법**
```
파이프라인 완료 후 "↓ ZIP 다운로드" 클릭 → http://localhost:3001/api/v1/projects/.../download 로 정상 이동
```
...
### 13. 파이프라인 Worker 미시작 + idea_input 누락 수정 (2026-04-20)
...
### 12. AgentArtifact @@unique 누락 → saveArtifact 500 오류 수정 (20### 26-04-20)
...
### 11. 파이프라인 실행 불가 버그 진단 (2026-04-20)

**프롬프트**
```
http://localhost:5173/pipeline/77c3d9ec-7928-43f3-8b09-eae70845f497 사이트 파이프라인 정상 실행여부 체크
```
...
| 서비스 | 상태 | 응답 |
|--------|------|------|
| Web (5173) | ✅ 정상 | HTTP 200 |
| API (3001) | ✅ 정상 | Fastify 실행 중 |
| PostgreSQL | ✅ 정상 | latency 71ms |
| Redis | ✅ 정상 | latency 10ms |
| Pipeline (8000) | ✅ 정상 | FastAPI 실행 중 |

**발견된 버그**

**Bug #1 (CRITICAL) — Worker가 시작되지 않음**
`server.js`에 `startPipelineWorker()` 호출 코드 없음.
프로젝트 생성 시 BullMQ 큐에 작업이 추가되지만 처리 워커가 없어 영원히 `pending` 유지.

```js
// apps/api/src/server.js에 추가 필요
import { startPipelineWorker } from './workers/pipeline.worker.js';
startPipelineWorker();
```

**Bug #2 (HIGH) — `idea_input` 누락**
`pipeline.worker.js`에서 Pipeline `/run` 호출 시 `project_id`만 전송, `idea_input` 누락.
Pipeline FastAPI는 `idea_input`을 필수값으로 요구.
...
// 수정 필요
{ project_id: projectId, idea_input: job.data.ideaInput }
```

**Bug #3 (MEDIUM) — Pipeline 서버 구버전 실행 중**
`main.py`에 `/run/until/{step}`, `/run/step/{step}` 라우트가 추가되어 있으나
현재 실행 중인 서버는 `/health`, `/run` 만 노출 — 서버 재시작 필요.

**수정 파일**
| 파일 | 작업 |
|------|------|
| `apps/api/src/server.js` | `startPipelineWorker()` 호출 추가 필요 |
| `apps/api/src/workers/pipeline.worker.js` | `idea_input` 전달 수정 필요 |

**확인 방법**
```bash
# 수정 후 API 서버 재시작
# 새 프로젝트 생성 → status가 running → completed 로 변경되면 정상
curl -s http://localhost:3001/api/v1/projects/{id} -b cookies.txt
```
...
### 15. 파이프라인 미진행 + 상태 정상표시 + 아웃풋 미표시 복합 수정 (2026-04-20)
...
### 16. 파이프라인 후반 에이전트 invoke_with_retry 누락 수정 (2026-04-20)
...
### 17. pipeline_logs 테이블 저장 누락 수정 (2026-04-20)
...
### 18. agent_artifacts 저장 0건 — pipeline 서버 .env 미로드 수정 (2026-04-20)
...
# 재시작 후 새 프로젝트 생성 → agent_artifacts 저장 확인
```
```sql
...
### 19. 파이프라인 진행상태 저장·복원 기능 개선 (2026-04-20)
...
### 20. 파이프라인 생성 즉시 fail — Docker 컨테이너 구버전·잘못된 API 키 수정 (2026-04-20)
